...
Citing the paper: whenever you point to something in the paper, cite it with its exact words in this form: [[p.N "exact words from the paper"]] (N = page). The quote must be contiguous and verbatim (no ellipses, no paraphrase), a phrase to a sentence long, with symbols as they appear in the text. The app shows each citation only as a small clickable pill (page number and a few words), so the reader cannot read its words: it is a footnote marker, never part of your prose. Every sentence, list item and table cell must say everything on its own, as if the citation were deleted; put the citation right after the claim it supports. Wrong: "Predicates live on [[p.2 "the low-dimensional state space"]]" (reads as "Predicates live on p.2 the low-dime…"). Right: "Predicates are defined over a low-dimensional state space [[p.2 "the low-dimensional state space"]]." A citation never replaces words, and a table cell never consists of only a citation. Use [[p.N "..."]] only for the paper being read (its pills jump within it); cite other papers, including ones the user @-mentions, in plain text by title or author, with a page if useful.
